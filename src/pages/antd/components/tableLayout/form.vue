    ...
    <a-form ref="formRef" :rules="rules" :model="formData" :name="name" layout="inline" class="comform">
        <template v-for="item in formItem">
            <a-form-item
                ...
                <!-- 下拉选择框 -->
                <a-select v-model:value="formData[item.key]" :placeholder="'请选择' + item.title" :allowClear="true" :disabled="item.disabled ? true : false" v-if="item.type === 'select'">
                ...
                <!-- 开关 -->
                <a-switch v-model:checked="formData[item.key]" checked-children="开" un-checked-children="关" :disabled="item.disabled ? true : false" v-if="item.type === 'switch'" />
                <!-- 单选框 -->
                <a-radio-group v-model:value="formData[item.key]" :options="item.options" :disabled="item.disabled ? true : false" v-if="item.type === 'radio'" />
                <!-- 多选框 -->
                <a-checkbox-group v-model:value="formData[item.key]" :options="item.options" :disabled="item.disabled ? true : false" v-if="item.type === 'checkbox'" />
                <!-- 图片上传 -->
                <upload v-model:value="formData[item.key]" :upload="item.upload" :disabled="item.disabled ? true : false" v-if="item.type === 'upload'" />
                <!-- 文件上传 -->
                <uploadFile v-model:value="formData[item.key]" :upload="item.upload" :disabled="item.disabled ? true : false" v-if="item.type === 'uploadFile'" />
                <!-- 自定义插槽 -->
